...
| **阅读位置与视窗** | 每个 PDF 读到第几页、每个 Excalidraw 画布停在哪（缩放/偏移） | PDF 的位置存在 pdf.js 自己那张表里，**多个 PDF 同时打开时会互相覆盖**；Excalidraw 的视窗压根不往文件里存，且它的「打开时缩放以适应」默认开着 |
...
3. **用户一动手就停手**：`click/keydown/wheel/touchstart/scroll` 之后 800ms 内一律不套用 —— 少了这条会把正在读第 1 页的人一把拽回第 40 页。
...
- **接管阅读位置与视窗**（关掉后该节其余选项隐藏）
  - 恢复阅读位置（总开关）
  - 接管 PDF / 接管 Excalidraw 绘图 / 接管核心 Canvas（默认关）
- **通用**：调试日志（控制台输出，Ctrl+Shift+I 查看）

两边各有「立即恢复 / 记住当前 / 清除记录」三个按钮。
...
- 折叠侧回归：98 项
- 视窗侧回归：139 项
- 合并本身（命令 id 不撞车、配置分离、一个定时器驱动两边、记录迁移、设置页分节、启动引导）：29 项
